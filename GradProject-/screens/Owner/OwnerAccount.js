...
function OwnerAccount({navigation, route}){
    ...
    const [myHall, setMyHallID] = useState('');
    const [image, setImage] = useState('https://icons.veryicon.com/png/o/internet--web/55-common-web-icons/person-4.png');
    useEffect(()=>{
            const HallID = query(collection(db, "Halls"), where("OwnerID", "==", userAccountCtx.userID));
            onSnapshot(HallID, (Halls) =>{
                setMyHallID(Halls.docs.map((Hall) =>({
                        id: Hall.id,
                        data: Hall.data()
                    }))
                )
    ...
    useEffect(()=>{
        if(myHall.length != 0){
            const dates = query(collection(db, "Reservation"), where("HallsID", "==", myHall[0].id));
    ...
    useEffect(()=>{
        if(numberOfReservations.length != 0){
            var total = 0;
            for(var i=0; i<numberOfReservations.length; i++){
                total = total + Number(numberOfReservations[i].data.Price)
            }
            setCollectedMoney(total);
        }
            ...
            <View style={styles.container}>
                ...
                <View style={styles.subContainer2}>
                    <View style={styles.informationContainer}>
                        ...
                        {
                            userAccountCtx.rule > 0 && userAccountCtx.rule < 1?
                                <View style={styles.emailContainer}>
                                    <View style={styles.email}>
                        ...
                        <View style={styles.bookContainer}>
                            <View style={styles.book}>
                                <Text style={styles.bookLabel}>Total Collected Money: </Text>
                                <Text style={styles.bookText}>{collectedMoney}</Text>
                            </View>
                        </View>
