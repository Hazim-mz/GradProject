...
function HallPage({route, navigation}){
    ...
    useEffect(()=>{
        ...
        if(userAccountCtx.isAuthenticated){
            const qqqqq = query(collection(db, "MessagesRequest"), where('OwnerID', '==', displayedHall.ownerID), where('UserID', '==', userAccountCtx.userID));
            onSnapshot(qqqqq, (Owners) =>
    ...
    function sendPushNotificationHandler(eventType){
        if(eventType == 'book'){
            if(owner[0].data.NotificationsAddress != ''){
                console.log(owner[0].data.NotificationsAddress);
