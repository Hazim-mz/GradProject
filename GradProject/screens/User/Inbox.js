...
function Inbox({navigation, route}){
    ...
    if(! userAccountCtx.isAuthenticated){
        ...
        for(var i=0; i<chats.length; i++){
            if(chats[i].data.Start){
                isThereChats=true;
                break;
            }
        }
        if(chats.length == 0 || !isThereChats){
            return(
                <View style={styles.noChatContainer}>
