...
function ChangePassword({oldPassword, idToken, changePasword, visible, close}){
    const [password, setPassword] = useState('');
    const [confirmPassword, setConfirmPassword] = useState('');
    const [isInvalidPssword, setIsInvalidPssword] = useState(false);
    const [isInvalidConfirm, setIsInvalidConfirm] = useState(false);

    async function onPress(){
        if(password == oldPassword ){
            setIsInvalidPssword(true);
            Alert.alert('Password it is same');
        }
        else if(password != confirmPassword){
            setIsInvalidConfirm(true);
            Alert.alert('Make sure that confirm password same to password');
        }else{
            const token = await updatePassword(idToken, password);
            changePasword(token.idToken);
            close();
        }
                        ...
                        <ScrollView style={{flex: 1}}>
                            ...
                            <View style={styles.authContent}>
                                <View style={styles.inputContainer}>
                                    <Text style={[styles.label, isInvalidPssword && styles.labelInvalid]}>
                                        New Password
                                    </Text>
                                    <TextInput
                                        style={[styles.input, isInvalidPssword && styles.inputInvalid]}
